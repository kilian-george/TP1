...
public class QuestionGui {
	...
	private StaffReviewGui staffReviewGui;
	...
	public Parent getView(Database database) throws SQLException {
		...
		this.staffReviewGui = new StaffReviewGui(database);
		...
		questionsList.setCellFactory(param -> new ListCell<Question>() {
			...
			protected void updateItem(Question item, boolean notHere) {
				...
				if (notHere || item == null) {
					setText(null);
					setGraphic(null);
				} else {
					Label questionLabel = new Label(item.getQuestionText());
					HBox qHbox = new HBox(10, questionLabel);
					if (database.getCurrentStaffRole()) {
						Button reviewButton = new Button("review question");
						reviewButton.setOnAction(e -> staffReviewGui.reviewQuestion(item));
						qHbox.getChildren().add(reviewButton);
					}
					setGraphic(qHbox);
				}
			}
	...
	/**
	 * Creates a button that allows the author of a question to edit its text.
	 * <p>
	 * If the current user is not the author of the question, the button is hidden.
	 * When clicked, a text area is presented allowing the user to edit the question.
	 * If the edited text is empty, the question is deleted from the database.
	 * Otherwise, the question text is updated and the detail view is refreshed.
	 * </p>
	 *
	 * @param question the {@code Question} object to be edited
	 * @return a {@code Button} that enables editing the given question
	 */
	...
	private List<Node> createQuestionData(Question question) {
		...
		questionLabel.setWrapText(true);
		// displays username
		Label nameLabel = new Label(question.getName());
	...
	private VBox createAnswersBox(Question question) {
		...
		if (question.getAnswers().isEmpty()) {
			...
			for (Answer a : sortAnswers) {
				...
				reviewButton.setVisible(database.getCurrentStaffRole());
				reviewButton.setOnAction(e -> staffReviewGui.reviewAnswer(a, question.getId()));
				// edit button
				Button editButton = new Button("Edit answer");
				...
				answersBox.getChildren().addAll(answerHorz, answerLabel, commentBox, reviewButton,
						editButton);
	...
	private VBox commentHelper(Answer answer, Question question) {
		...
		List<Comment> answersComment = database.getCommentsByAnswerId(answer.getId());
		answer.setComments(answersComment);
		for (Comment comment : answersComment) {
			// looks for all the comments and makes timestamps for them
			LocalDateTime time0 = comment.getTimestamp();
			String time = time0.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
			Label commentLabel = new Label("-By:" + comment.getName() 
				+ " " + comment.getText() + " (" + time + ") ");
			HBox commentRow = new HBox(10, commentLabel);
			commentBox.getChildren().add(commentLabel);

			if (database.getCurrentStaffRole()) {
				Button reviewButton = new Button("review this comment");
				reviewButton.setOnAction(e -> staffReviewGui.reviewComment(comment, answer.getId()));
				commentRow.getChildren().add(reviewButton);
			}
		...
		commentButton.setOnAction(e -> {
			...
			if (!text.isEmpty()) {
				Comment insertComment = new Comment(database.getCurrentUsername(), text);
				database.saveComment(answer.getId(), insertComment);
				answer.addComment(insertComment);
	...
	/**
	 * Creates a custom search input field for filtering and selecting questions
	 * from a provided list.
	 * <p>
	 * As the user types, a popup dropdown displays up to five questions ranked by
	 * keyword match. Users can navigate suggestions using the keyboard or mouse and
	 * select a question to trigger a callback.
	 *
	 * @param existingQuestions  the list of {@code Question} objects to search
	 *                           through
	 * @param onQuestionSelected a {@code Consumer} that will be invoked with the
	 *                           selected {@code Question} when a suggestion is
	 *                           clicked or confirmed via the Enter key
	 * @return a {@code Node} (specifically a {@code TextField}) representing the
	...
	private Node customQuestionSearchField(List<Question> existingQuestions, Consumer<Question> onQuestionSelected) {
		...
		List<String> allQuestions = existingQuestions.stream().map(Question::getQuestionText)
				.collect(Collectors.toList());
		...
		inputField.textProperty().addListener((obs, oldVal, newVal) -> {
			...
			// splits the words into a set on whitespace
			Set<String> inputWords = Arrays.stream(newVal.toLowerCase().split("\\W+")).filter(word -> !word.isBlank())
					.collect(Collectors.toSet());
			// creates an internal score of matches to their question
			List<String> rankedMatches = allQuestions.stream().map(q -> {
				Set<String> qWords = Arrays.stream(q.toLowerCase().split("\\W+")).collect(Collectors.toSet());
				...
				return Map.entry(q, score);
					...
					// gets the top 5 scoring matches based on word
					.map(Map.Entry::getKey).limit(5).collect(Collectors.toList());

			suggestionList.getItems().setAll(rankedMatches);
			// if the popup has something to show it will according to the bounds of the
			// screen
			if (!popup.isShowing() && !rankedMatches.isEmpty()) {
				...
				// otherwise the pop up will go away
		...
		suggestionList.setOnMouseClicked(event -> {
			...
			if (selected != null) {
				// if the click is on one of the questions then it queues it
				inputField.setText(selected);
				popup.hide();
				existingQuestions.stream().filter(q -> q.getQuestionText().equals(selected)).findFirst()
		...
		inputField.setOnKeyPressed(event -> {
			if (event.getCode() == KeyCode.DOWN) {
				...
				// keying in the enter key will imput the suggestion
