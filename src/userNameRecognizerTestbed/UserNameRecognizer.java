...
public class UserNameRecognizer {
	...
	private static boolean running;						// The flag that specifies if the FSM is 
	                                                    // running
	private static boolean otherChar;                   //If a special char outside of allowed
	private static boolean alpha;						// determines if the previous char is a 
														//letter instead of anything else
	private static char nextChar;						//the next char in line
	...
	public static String checkForValidUserName(String input) {
		...
		alpha=true;							//starts with true otherwise fail state during state 1
		...
		while (running) {
			...
			case 0: 
				...
				// The current character is checked against A-Z, a-z, 0-9. If A-Z and a-z are matched
			...
			case 1: 
				...
				// symbol -> State 2
					...
					nextState = 2;
					otherChar = true;			//it is an allowed special character
					// Count the symbol
					userNameSize++;
				}			
				// If it is none of those characters, the FSM halts
				else {
					otherChar=false;		//Disallowed special character
					running = false;
				}
				// The execution of this state is finished
			...
			case 2: 
				// State 2 deals with a character after a symbol in the name.
		...
		case 1:
			...
			if(!otherChar) {
				userNameRecognizerErrorMessage +=
						"The only allowed special characters are period, dash, and underscore.";
				return userNameRecognizerErrorMessage;
			}
