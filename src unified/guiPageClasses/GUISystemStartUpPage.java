...
public class GUISystemStartUpPage {
	...
	**********************************************************************************************/
	...
	private void doSetupAdmin() {
			String useroutput = UserNameRecognizer.checkForValidUserName(adminUsername);
			//checks to see if a username is valid and if not
			//creates a popup from the error derived from
			//UserNameRecognizer
			if(!useroutput.isEmpty()) {
				Alert error = new Alert(AlertType.ERROR);
				error.setContentText(useroutput);
				error.showAndWait();
				adminUsername = "";
				text_AdminUsername.clear();
				return;
		}
		String passoutput = PasswordEvaluator.evaluatePassword(adminPassword1);
		//checks to see if the password is valid and if not 
		//creates a popup from the error derived from the 
		//passwordEvaluator
		if(!passoutput.isEmpty()) {
			Alert error = new Alert(AlertType.ERROR);
			error.setContentText(passoutput);
			error.showAndWait();
			adminPassword1="";
			adminPassword2="";
	...
	private void doLogin() throws SQLException {		
		...
		//creates a popup from the error derived from
		//UserNameRecognizer
		String output = UserNameRecognizer.checkForValidUserName(username);
		if(!output.isEmpty()) {
			Alert error = new Alert(AlertType.ERROR);
		...
		String passOut = PasswordEvaluator.evaluatePassword(password);
		//checks to see if the password is valid and if not 
		//creates a popup from the error derived from the 
		//passwordEvaluator
		if(!passOut.isEmpty()) {
			Alert error = new Alert(AlertType.ERROR);
			error.setContentText(passOut);
			error.showAndWait();
			password = "";
			text_Password.clear();
		}
 ...
 */   	
	...
	private void doCreateUser(String role, String iCode) {
		...
		String useroutput = UserNameRecognizer.checkForValidUserName(username);
		//checks to see if the username is valid, if not it pops up an error
		if(!useroutput.isEmpty()) {
			System.out.println("inside useroutput\n");
			Alert error = new Alert(AlertType.ERROR);
			error.setContentText(useroutput);
			error.showAndWait();
			username = "";
		...
		String passoutput = PasswordEvaluator.evaluatePassword(password);
		//checks to see if the password is valid, if not returns an error in a popup
		if(!passoutput.isEmpty()) {
			Alert error = new Alert(AlertType.ERROR);
			error.setContentText(passoutput);
			error.showAndWait();
